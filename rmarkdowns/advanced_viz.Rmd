...
```{r, message=FALSE}
advanced_viz <- read_csv(here("data/SDR-2023-Data.csv"))
```
...
```{r, warning = FALSE}
ggplot(data = advanced_viz, aes(x= goal_6_score, y = goal_4_score, color = regions_used_for_the_sdr)) +
  geom_point()
```
```{r}
goal_4_scatter <- ggplot(advanced_viz, aes(x = goal_6_score,y = goal_4_score, color=regions_used_for_the_sdr, lebel = country)) +
  geom_point() +
  theme_minimal() +
  scale_fill_viridis_d(option = "magma") +
  labs(title = "Scores Bruh!",
       x = "SDG 6 Score",
       y = "SDG 4 score",
       color = "Legend")
...
colnames(advanced_viz)[which(colnames(advanced_viz) == "country_code_iso3")] <- "iso_a3"
```
...
```{r}
advanced_viz_world_joined <- left_join(advanced_viz, world, by = "iso_a3")
```
...
```{r}
class(advanced_viz_world_joined)
```
...
```{r}
advanced_viz_world_joined <- st_as_sf(advanced_viz_world_joined)
```
...
```{r}
advanced_viz_world_joined <- st_transform(advanced_viz_world_joined, "+proj=longlat +datum=WGS84")
```
...
```{r}
mytext <- paste(
    "Country: ", advanced_viz_world_joined$country,"<br/>", 
    "Goal 7 Score: ", round(advanced_viz_world_joined$goal_7_score, 2), 
    sep="") %>%
  lapply(htmltools::HTML)

leaflet(advanced_viz_world_joined) %>% 
  addTiles()  %>% 
...
mytext <- paste(
    "Country: ", advanced_viz_world_joined$country,"<br/>", 
    "Goal 6 Score: ", round(advanced_viz_world_joined$goal_6_score, 2), 
    sep="") %>%
  lapply(htmltools::HTML)
...
leaflet(advanced_viz_world_joined) %>% 
  addTiles()  %>% 
  setView( lat=10, lng=0 , zoom=2) %>%
  addPolygons(stroke = FALSE, fillOpacity = 0.5, smoothFactor = 0.5, color = ~colorQuantile("Paired", goal_6_score)(goal_6_score), label = mytext)
```
