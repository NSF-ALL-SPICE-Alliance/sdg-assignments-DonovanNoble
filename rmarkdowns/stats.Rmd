...
```{r}
sdg_correlation <- ggplot(sdr_data, aes(x = goal_6_score, 
                     y = goal_7_score, label = country)) +
  geom_point() +
  geom_smooth() +
...
```
INTERPRETATION:
In this graph, it shows that these two goals have a positive correlation. Goal 6 is clean water and sanitation, goal 7 is affordable clean energy. Both goals are very similar in affecting the environment. Most countries are very close in scores between goals, which is the reason for a overall positive correlation. There are a few outlier countries, such as; Chad, South Sudan, Iceland, and Uruguay. These are the countries that have a wider range of scores between goals. 
