...
```{r}
ggplot(data = sdr_data, aes(x= goal_14_score, y = goal_15_score, color = goal_16_score))+
  geom_point()
```
Interpretation:
In this graph within goal score 14 to 16. All scores are mostly targeted around 60 to 70. There are a few other chunks of scores but as we can see there is not much for achieving 100 in these goals. 
### Bar Chart
...
```{r}
ggplot(lac_sdr_data, aes(x= goal_3_score, y = country, color = goal_6_score)) +
  geom_bar(stat = "identity") 
```
...
```{r}
ggplot(MENA_sdr_data, aes(x= goal_3_score, y = country, color = goal_5_score)) +
  geom_boxplot()
```
Interpertation:
According to this graph on the scores from only the MENA region, all the country are able to reach above a score of 50 besides Yemen. United Arab Emirates, Qatar, Kuwait, and Bahrain are the top leading countries in keeping close to their goals. 
### Histogram
...
```
```{r}
ggplot(OECD_sdr_data, aes(x = goal_4_score, fill=regions_used_for_the_sdr)) + 
  geom_histogram(bins=33, color="black")
```
Interpretation:
This is a graph based on the region of OECD. This region has a lot of different countries within in, and as a collective most countries in this region is reaching high scores. 
